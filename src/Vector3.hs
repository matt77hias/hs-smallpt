...
mind_v3 :: Vector3 -> Int
mind_v3 (Vector3 x y z) = if ((x < y) && (x < z)) then 1 else (if (y < z) then 2 else 3)

maxd_v3 :: Vector3 -> Int
maxd_v3 (Vector3 x y z) = if ((x > y) && (x > z)) then 1 else (if (y > z) then 2 else 3)

min_v3 :: Vector3 -> Double
min_v3 (Vector3 x y z) = min x $ min y z

max_v3 :: Vector3 -> Double
max_v3 (Vector3 x y z) = max y $ max y z
